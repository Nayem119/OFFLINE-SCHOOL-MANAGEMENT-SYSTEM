...
class PersonFrame(ttk.Frame):
    def __init__(self, master, user):
        ...
        self.fields=[('Staff ID', 'staff_id'), ('নাম', 'name'), ('Designation', 'designation'), ('Phone', 'phone'), ('Email', 'email'), ('Address', 'address'), ('Joining Date', 'joining_date'), ('Salary', 'salary'), ('Status', 'status')]
        self.vars={k:tk.StringVar() for _,k in self.fields}
        self.vars["status"].set("Active")
        self.build(); self.load()

    def build(self):
        box=ttk.LabelFrame(self,text="👨‍💼 Staff Management",padding=10); box.pack(fill="x")
        ...
        ttk.Button(a,text="➕ Add Staff",command=self.popup_add).pack(side="left",padx=3)
        ttk.Button(a,text="✏️ Edit Staff",command=self.popup_edit).pack(side="left",padx=3)
        ...
        self.tree=ttk.Treeview(self,columns=('id', 'name', 'designation', 'phone', 'salary', 'status'),show="headings")
        for c,h in zip(('id', 'name', 'designation', 'phone', 'salary', 'status'),('ID', 'নাম', 'Designation', 'Phone', 'Salary', 'Status')):
    ...
    def load(self):
        ...
        rows=query("SELECT * FROM staff WHERE name LIKE ? OR phone LIKE ? OR staff_id LIKE ? ORDER BY id DESC",(term,term,term))
        for r in rows:
            self.tree.insert("", "end",values=tuple(r[c] for c in ('id', 'name', 'designation', 'phone', 'salary', 'status')))

    def popup_add(self):PopupForm(self,"Add Staff",[(label,key,["Active","Inactive","On Leave","Retired"] if key=="status" else None) for label,key in self.fields],on_save=lambda values:self.popup_save(values,False))
    def popup_edit(self):
        if not self.selected_id:return messagebox.showwarning("Select","আগে একটি record নির্বাচন করুন।")
        PopupForm(self,"Edit Staff",[(label,key,["Active","Inactive","On Leave","Retired"] if key=="status" else None) for label,key in self.fields],self.values(),on_save=lambda values:self.popup_save(values,True))
    ...
    def add(self):
        ...
        ident=v["teacher_id" if "staff"=="teachers" else "staff_id"]
        ...
        try:
            ...
            execute("INSERT INTO staff ("+",".join(cols)+") VALUES ("+placeholders+")",vals)
            log_activity(self.user["username"],"Added Staff "+ident)
            messagebox.showinfo("Success","Staff যোগ হয়েছে।");self.clear();self.load()
    ...
    def select(self,_=None):
        ...
        key="teacher_id" if "staff"=="teachers" else "staff_id"
        rows=query("SELECT * FROM staff WHERE "+key+"=?",(ident,))
        if rows:
            r=rows[0];self.selected_id=r["id"]
            for k in self.vars:self.vars[k].set(r[k] if k in r.keys() and r[k] is not None else "")

    def show_profile(self):
        if not self.selected_id:return messagebox.showwarning("Select","আগে একজন staff নির্বাচন করুন।")
        record=dict(query("SELECT * FROM staff WHERE id=?",(self.selected_id,))[0])
        fields=[("Staff ID","staff_id"),("Name","name"),("Designation","designation"),("Phone","phone"),("Email","email"),("Address","address"),("Joining Date","joining_date"),("Salary","salary"),("Status","status")]
        open_profile(self,"Staff Profile - "+record["name"],record,fields)
    ...
    def update(self):
        ...
        try:
            ...
            execute("UPDATE staff SET "+sets+" WHERE id=?",list(v.values())+[self.selected_id])
            log_activity(self.user["username"],"Updated Staff "+v["name"])
    ...
    def delete(self):
        ...
        if not require_admin(self.user, messagebox, "Staff delete"):return
        if not messagebox.askyesno("Confirm","এই record Delete করবেন?"):return
        execute("DELETE FROM staff WHERE id=?",(self.selected_id,))
        log_activity(self.user["username"],"Deleted Staff")
